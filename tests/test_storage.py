...
class TestStorageEngine(unittest.TestCase):
    def setUp(self):
        ...
        # Mocking os.environ
        self.mocked_environ = patch.dict(
            os.environ,
            {
    ...
    def test_get_object(self):
        ...
        self.s3_client.put_object(
            Bucket=self.bucket_name, Key=test_file_key, Body=b"Test file content"
        )
    ...
    def test_upload_object_local(self):
        ...
        self.storage_engine.upload_object_local(
            local_path=test_file_path,
            cloud_path="test/file.txt",
    ...
    def test_upload_object(self):
        ...
        response = self.s3_client.get_object(
            Bucket=self.bucket_name, Key="tests/file.txt"
        )
        self.assertEqual(response["Body"].read(), b"Test file content")
...
class TestStoreEngineMultiFile(unittest.TestCase):
    def setUp(self):
        ...
        self.job_id = "123"
        self.store_engine = StoreEngineMultiFile(
            self.job_id, bucket=self.bucket_name, resource=self.resource
        )
...
class TestStorageEngineDownloader(unittest.TestCase):
    def setUp(self):
        ...
        self.client.put_object(
            Bucket=self.bucket_name, Key="test-key", Body=b"Test file content"
        )

        self.storage_downloader = StorageEngineDownloader(
            bucket=self.bucket_name, client=self.client, resource=self.resource
        )
    ...
    def test_upload_in_memory_object(self):
        ...
        self.client.upload_fileobj.assert_called_once_with(
            in_memory_object, self.bucket_name, output_file
        )
    ...
    def test_copy_objects(self):
        ...
        self.client.copy_object.assert_called_once_with(
            Bucket=self.bucket_name,
            Key=destination_key,
            CopySource=f"bucket-test/{source_key}",
        )
    ...
    def test_create_arrangement_file(self):
        ...
        with patch.object(pydub.AudioSegment, "from_file") as mock_from_file:
            ...
            self.storage_downloader.create_arrangement_file(
                ["file1", "file2"], format="wav"
            )
    ...
    def test_get_presigned_url(self):
        ...
        # Call the method under test
        presigned_url = self.storage_downloader.get_presigned_url(
            "test_file.txt", expires_in=100
        )
    ...
    def test_upload_and_get_presigned_url(self):
        ...
        # Call the method under test
        presigned_url = self.storage_downloader.upload_and_get_presigned_url(
            "test.zip", in_memory_zip, expires_in=300
        )
        ...
        self.client.upload_fileobj.assert_called_once_with(
            in_memory_zip, self.bucket_name, "test.zip"
        )
        self.client.generate_presigned_url.assert_called_once_with(
            "get_object",
...
class TestSnapshotManager(unittest.TestCase):
    def setUp(self):
        ...
        for i in range(5):
            self.client.put_object(
                Bucket=self.bucket_name, Key=f"file{i}.txt", Body=b"some data"
            )
    ...
    def test_build_snapshot(self):
        ...
        csv_obj = self.resource.Object(
            self.snapshot_manager.SNAPSHOTS_DUMP_BUCKET, s3_key
        ).get()["Body"]
    ...
    def test_get_snapshot_data_success(self, mock_save, mock_process, mock_load):
        ...
        snapshot_manager = SnapshotManager(
            bucket="test_bucket", client=None, resource=None
        )
    ...
    @patch.object(
        SnapshotManager, "load_snapshot_from_s3", side_effect=ClientError({}, "")
    )
    ...
    def test_load_snapshot_from_s3(self, mock_read_csv):
        self.resource.Object(self.bucket_name, "database_snapshot.csv").put(
            Body="some data"
        )
    ...
    def test_process_snapshot_data(self):
        ...
        data = {
            ...
            "other_column": [
                ...
                4,
                5,
            ],  # just an example, replace with actual other columns
        }
        ...
        assert all(
            ~self.snapshot_manager.snapshot_files_df["label"].isin(
                self.snapshot_manager.FILTER_OUT_LABELS
            )
        )
    ...
    def test_save_snapshot_files_to_s3(self):
        ...
        self.snapshot_manager.snapshot_files_df = pd.DataFrame(
            {"A": [1, 2, 3], "B": [4, 5, 6]}
        )
        ...
        mock_s3_resource.Object.assert_called_once_with(
            self.snapshot_manager.SNAPSHOTS_DUMP_BUCKET,
    ...
    def test_generate_presigned_urls(self):
        ...
        # Then
        params1 = {
            "Bucket": self.snapshot_manager.SNAPSHOTS_DUMP_BUCKET,
            "Key": self.snapshot_manager.SNAPSHOT_CSV,
        }
        params2 = {
            "Bucket": self.snapshot_manager.SNAPSHOTS_DUMP_BUCKET,
            "Key": self.snapshot_manager.SNAPSHOT_FILES_CSV,
        }
        mock_s3_client.generate_presigned_url.assert_any_call(
            "get_object", Params=params1, ExpiresIn=3600
        )
        mock_s3_client.generate_presigned_url.assert_any_call(
            "get_object", Params=params2, ExpiresIn=3600
        )
