...
class TestUserActivityDB(unittest.TestCase):
    ...
    def test_get_favourite_sessions(self, mock_db_conn):
        ...
        """
        mock_cursor.execute.assert_called_once_with(query, (test_user_id,))
    ...
    def test_update_favourite_arrangements(self, mock_db_conn):
        ...
        # When
        user_activity_db.update_favourite_arrangements(
            updated_data, "test_user", "test_session"
        )
    ...
    def test_update_favourite_sessions(
        self, mock_insert_to_favourite_sessions, mock_db_conn
    ):
        # Given
        mock_cursor = MagicMock()
        mock_db_conn.return_value.cursor.return_value.__enter__.return_value = mock_cursor
        mock_cursor.fetchall.return_value = (
            []
        )  # simulate situation where no session record exists
        user_activity_db = UserActivityDB()
        ...
        # Then
        mock_insert_to_favourite_sessions.assert_called_once_with(
            "test_user", "test_session"
        )  # Check if 'insert_to_favourite_sessions' is called.
    ...
    def test_transform_file_names(self):
        ...
        user_activity_db = UserActivityDB()
        file_names = [
            "temp/2022_session1/2022_session1_master.wav",
            "temp/2022_session2/2022_session2_master.mp3",
        ]  # make sure these match the filter conditions
