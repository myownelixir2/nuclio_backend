...
class JobTypeValidator(BaseModel):
    job_type : Literal['job_id_path', 'processed_job_path', 'asset_path', 'mixdown_job_path', 'mixdown_job_path_pkl']
    
    @validator('job_type')
    def job_type_validator(cls, v):
        if v not in ['job_id_path', 'processed_job_path', 'asset_path', 'mixdown_job_path', 'mixdown_job_path_pkl']:
            raise ValueError('job_type must be either "job_id_path", "processed_job_path", "asset_path", "mixdown_job_path" or "mixdown_job_path_pkl"')
        return v
...
class JobConfig:
    ...
    def path_resolver(self):
        ...
        local_path_mixdown = f'temp/mixdown_{random_id}_{sanitized_job_id}'
        cloud_path_mixdown = f'mixdown/mixdown_{random_id}_{sanitized_job_id}'
        
        local_path_mixdown_pkl = f'{local_path_mixdown}.pkl'
        cloud_path_mixdown_pkl = f'{cloud_path_mixdown}.pkl'
        
        local_path_mixdown_mp3 = f'{local_path_mixdown}.mp3'
        cloud_path_mixdown_mp3 = f'{cloud_path_mixdown}.mp3'
